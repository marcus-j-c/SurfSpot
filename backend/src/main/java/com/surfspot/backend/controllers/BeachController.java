...
public class BeachController {
    ...
    private GeocodingInfo coordsRequest(String name) {
        ...
        if (openMeteoResponse != null && openMeteoResponse.results() != null && !openMeteoResponse.results().isEmpty()) { //check if the response isnt empty.
            ...
            return new GeocodingInfo(sortOpenMeteoResponse); // if it isnt return it.
        }
        ...
        for (String suffix: suffixes) {
            if (doubleCleanedName[doubleCleanedName.length - 1].equalsIgnoreCase(suffix)) {
                ...
                return coordsRequest(cleanedNameWithoutSuffix); //recursively call coordsRequest with the cleaned name without the suffix
            }
        }
