...
> Note: The backend is hosted on a free tier of render and may take 50+ seconds to wake up on the first request after not being used. Afterwards, searches will be fast.
...
2. Locate & Fetch Conditions: SurfSpot finds the spot's exact coordinates, then gathers the current conditions of that spot using external APIs. (Unless the spot is a cache hit)
...
- Currently the trending beaches sidebar is not dynamic, because SurfSpot is only connected to a caching database, and so it carries out real searches (clicking Bells Beach is the same as searching it in the search bar), but the list doesn't change and popularity data is hardcoded.
...
- Due to SurfSpot only having a database for caching, the account buttons are currently also non functional, in future these will be used, and account info will affect the rating algorithm.
