...
User inputted beach name -> Geocoding -> Coordinates -> Marine and weather data from open-meteo -> Rating algorithm -> React UI
...
4. No API keys required because SurfSpot uses Open-Meteo and Nominatim, both of which, have free, key less tiers.

## Known Limitations (Future Additons In V2)
...
- Due to SurfSpot not having a database, API results cannot be cached, in future versions, I hope to only update data every hour, or whenver the spot is searched, whatever is less frequent, in order to save API tokens.
